...
std::vector<int> compress_row(const std::vector<int>& row) {
    ...
    copy_if(row.begin(), row.end(), back_inserter(compressed), [](int val){
        return val != 0;
    });
...
bool move_left(std::vector<std::vector<int>>& board) {
    ...
    for (int r = 0; r < 4; r++) {
        vector<int> original = board[r];
        vector<int> compressed = compress_row(original);
...
bool move_right(std::vector<std::vector<int>>& board) {
    ...
    for (int r = 0; r < 4; r++) {
        ...
        vector<int> compressed = compress_row(temp);
        vector<int> merged = merge_row(compressed);
...
bool move_up(std::vector<std::vector<int>>& board) {
    ...
    for (int c = 0; c < 4; c++) {
        ...
        for (int r = 0; r < 4; r++)
            col[r] = board[r][c];
        vector<int> original = col;
        vector<int> compressed = compress_row(col);
        vector<int> merged = merge_row(compressed);
        if (original != merged)
            moved = true;
        for (int r = 0; r < 4; r++)
